...
export const SUPPORTED_CODES = SUPPORTED_CURRENCIES.map(c => c.code) as [CurrencyCode, ...CurrencyCode[]];
...
export const CODE_CURRENCY = new Map<CurrencyCode, Readonly<CurrencyType>>(SUPPORTED_CURRENCIES.map(c => [c.code, c]));
export const FALLBACK_CODE_CURRENCY = CODE_CURRENCY.get(DEFAULT_CURRENCY) ?? SUPPORTED_CURRENCIES[0];

export const TZ_CURRENCY: Record<string, CurrencyCode> = {
